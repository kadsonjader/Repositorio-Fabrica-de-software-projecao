...
$listaArq = $arqDAO -> Listar();

if(isset($_GET['a']) && $_GET['a'] == 'd'){
    $cont = 0;
    for($i = 0; $i < count($listaArq); $i++){
        if($listaArq[$i]->getUsuario_idUsuario() == $_GET['id']){
            $cont++;
        }
    }
    if($cont == 0){
        ...
        header("Location: ?pg=usuario&a=N");
    }
}else{
    if(isset($_GET['a']) && $_GET['a'] == 'st'){
				...
				<div class="row">
					<div class="col-md-12 col-sm-12">
                        <div class="content">
							<div class="alert alert-danger">
								<button type="button" aria-hidden="true" data-dismiss="alert" class="close">×</button>
									<span>Não foi possível excluir. Existem arquivos associados à este usuário.</span>
							</div>
						</div>
					</div>
				</div>
...
?>
										<tr>
											...
											<td><?php if($listaUsuarios[$i]->getStatus() == 'A'){ ?>
												<a href="?pg=usuario&a=st&id=<?php echo $listaUsuarios[$i]->getIDUsuario(); ?>">
